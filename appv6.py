...
if current_popularity_element:
    ...
    if parent_div:
        ...
        for div in child_divs:
            aria_label_parts = div['aria-label'].split(' ')
            if len(aria_label_parts) >= 3:
                time_label = aria_label_parts[2]  # Extract the time from the aria-label
                popularity = aria_label_parts[0]  # Extract the popularity percentage
                data[time_label] = popularity
